...
public class BSConfigShop extends BSShop {
    ...
    public BSConfigShop(int shop_id, File f, BSShops shophandler, ConfigurationSection sectionOptional) {
        ...
        try {
            ...
            m.setLore(lore);
            i.setItemMeta(m);
            addShopItem(new BSBuy(BSRewardType.Command, BSPriceType.Nothing, "tell %player% the config managers (" + ymlName + ") contains mistakes...", null, "", 0, "", name), i, ClassManager.manager);
            finishedAddingItems();
            return;
        }
